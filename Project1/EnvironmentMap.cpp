	...
	loadedUniforms(false)
{
	...
	glTextureStorage2D(GL_TEXTURE_CUBE_MAP, 10, GL_RGBA8, width, height);

	this->width = width;
	this->height = height;

	glTextureSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, 0, 0, this->width, this->height, imageFormat, GL_UNSIGNED_BYTE, posXData);
	...
	glTextureSubImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, 0, 0, this->width, this->height, imageFormat, GL_UNSIGNED_BYTE, negXData);
	...
	glTextureSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, 0, 0, this->width, this->height, imageFormat, GL_UNSIGNED_BYTE, posYData);
	...
	glTextureSubImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, 0, 0, this->width, this->height, imageFormat, GL_UNSIGNED_BYTE, negYData);
	...
	glTextureSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, 0, 0, this->width, this->height, imageFormat, GL_UNSIGNED_BYTE, posZData);
	...
	glTextureSubImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, 0, 0, this->width, this->height, imageFormat, GL_UNSIGNED_BYTE, negZData);
...
void EnvironmentMap::Draw(Ref<Shader> shader, const glm::vec3& position, const glm::vec3& scale)
{
	...
	glUniform1i(this->cubeSamplerUniform1, textureUnit);
	glUniform1i(this->cubeSamplerUniform2, textureUnit);
	glUniform1i(this->cubeSamplerUniform3, textureUnit);
...
Ref<EnvironmentMap> EnvironmentMap::StaticLoad(const YAML::Node& node)
{
	...
	test.as<std::string>();
	std::string posXPath = SerializeUtils::LoadPath(node["PosXPath"].as<std::string>());
	std::string negXPath = SerializeUtils::LoadPath(node["NegXPath"].as<std::string>());
	std::string posYPath = SerializeUtils::LoadPath(node["PosYPath"].as<std::string>());
	std::string negYPath = SerializeUtils::LoadPath(node["NegYPath"].as<std::string>());
	std::string posZPath = SerializeUtils::LoadPath(node["PosZPath"].as<std::string>());
	std::string negZPath = SerializeUtils::LoadPath(node["NegZPath"].as<std::string>());
	bool mipMaps = node["MipMaps"].as<bool>();
	bool seamless = node["Seamless"].as<bool>();
	std::string meshPath = SerializeUtils::LoadPath(node["MeshPath"].as<std::string>());
